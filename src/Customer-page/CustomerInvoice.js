...
import { Box, Container, Paper, Typography, Grid, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button, Divider, Chip, CircularProgress } from "@mui/material";
    ...
    <Container maxWidth="md" sx={invoiceStyles.container}>
      <Box sx={{ ...invoiceStyles.actionBox, display: "flex", gap: 2, justifyContent: "space-between", flexWrap: "wrap" }}>
        ...
        <Box sx={{ display: "flex", gap: 1.5 }}>
          ...
          <Button variant="contained" disableElevation startIcon={<Print />} onClick={handlePrint} sx={{ borderRadius: 2 }}>
            Print Invoice
          </Button>
        </Box>
      </Box>
        ...
        <Paper sx={invoiceStyles.paper}>
          ...
          <Grid container spacing={3} sx={{ mb: 6 }}>
            ...
            <Grid item xs={12} sm={6} sx={{ textAlign: { sm: "right" }, mt: { xs: 2, sm: 0 } }}>
              <Typography variant="h4" fontWeight="300" color="text.secondary" sx={{ letterSpacing: 2 }}>
                INVOICE
              </Typography>
              <Typography variant="body2" sx={{ mt: 1 }}>
                <strong>Invoice No:</strong> #{bill?.id || id}
              </Typography>
              <Typography variant="body2">
                <strong>Date:</strong> {new Date().toLocaleDateString()}
              </Typography>
              <Typography variant="body2">
                <strong>Due Date:</strong> {bill?.date ? new Date(bill.date).toLocaleDateString() : "N/A"}
              </Typography>
            </Grid>
          </Grid>
            ...
            <Table>
              <TableHead sx={invoiceStyles.tableHead}>
                <TableRow>
                  ...
                  <TableCell align="center" sx={invoiceStyles.tableHeaderCell}>
                    Rental Price
                  </TableCell>
                  <TableCell align="right" sx={invoiceStyles.tableHeaderCell}>
                    Deposit
                  </TableCell>
                  <TableCell align="right" sx={invoiceStyles.tableHeaderCell}>
                    Fine Amount
      ...
      <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end", mt: 3, mb: 5 }}>
        {bill?.status !== "canceled" && (
          <>
            <Button variant="outlined" color="error" size="large" startIcon={<Cancel />} onClick={handleCancelInvoice} sx={{ px: 4, borderRadius: 2, fontWeight: "bold", textTransform: "none" }}>
              Cancel Invoice
            </Button>
            <Button variant="contained" color="success" size="large" disableElevation startIcon={<Payment />} sx={{ px: 5, borderRadius: 2, fontWeight: "bold", textTransform: "none" }}>
              Proceed to Payment
            </Button>
          </>
        )}
