      ...
      <Paper elevation={2} sx={{ p: 4, borderRadius: "16px", mb: 4, backgroundColor: "#ffffff", boxShadow: "0px 4px 20px rgba(0, 0, 0, 0.05)" }}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth size="small" type="date" name="from_date" value={filters.from_date} onChange={handleChange} label="Published From" InputLabelProps={{ shrink: true }} />
          </Grid>

          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth size="small" type="date" name="to_date" value={filters.to_date} onChange={handleChange} label="Published To" InputLabelProps={{ shrink: true }} />
          </Grid>
        </Grid>
      ...
      <Box sx={{ mt: 4 }}>
        ...
        {hasSearched && !loading && results.length === 0 && (
          <Paper sx={{ p: 5, textAlign: "center", borderRadius: "12px", boxShadow: "none", border: "1px solid #e2e8f0" }}>
            ...
            ><Link style={{textDecoration:"none",fontSize:"15px",color:"white",fontWeight:"600"}} to="/BookRequest">If you went Order the book click here</Link></Button>
          </Paper>
        )}
            ...
            {results.map((book) => (
              <Grid item xs={12} sm={6} md={4} key={book.id} sx={{ display: "flex", justifyContent: "center" }}>
                <Box sx={{ width: "100%", maxWidth: "340px" }}>
                  <OutlinedCard showSnackbar={showSnackbar} rent={true} book={book} />
                </Box>
              </Grid>
            ))}
          </Grid>
